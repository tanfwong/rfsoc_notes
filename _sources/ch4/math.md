...
* Vitis HLS supports the standard C/C++ `float` and `double` types as
  ...
  * - `double`
  ...
  example](https://github.com/Xilinx/Vitis-HLS-Introductory-Examples/blob/master/Modeling/using_float_and_double/fp_mul_pow2.h)
...
* The Vitis HLS Math Library `hls_math` implements synthesizable
  ...
  - Since the bit-approximate implementation of a function in the
    ...
    test bench golden results in the co-simulation step, we should always
...
* Starting from v2024.2, Vitis HLS also supports arbitrary precision float
  ...
  point types can be found
  [here](https://docs.amd.com/r/en-US/ug1399-vitis-hls/Overview-of-Arbitrary-Precision-Fixed-Point-Data-Types).
...
* **Example**:
  ...
  typedef hls::vector<int, 100> vint100;
  
  void vadd(vint100 a, vint100 b, vint100 &c) {
    c = a+b;
  }
  ```
  Vitis HLS synthesizes 100 adders and 200 registers to hold
  and add the elements of the vectors `a` and `b` in parallel. The RTL
  ...
  completely unrolled and the arrays completely reshaped. The SIMD
  addition can be performed in a single clock cycle using a
  large amount of PL resources.
