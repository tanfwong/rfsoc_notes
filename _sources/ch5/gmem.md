...
* The figure below shows how the DSP kernel's `m_axi` interface
  ...
  ```
  - During a burst access, a read request is sent to the kernel's
    `m_axi` interface adapter, which serves as a buffer for all read
    (write) requests generated by the kernel. The `m_axi` adapter may
  ...
  - Overall, the read (write) latency is generally non-deterministic
    ...
    cycles. Thus, burst access can significantly increase the global
