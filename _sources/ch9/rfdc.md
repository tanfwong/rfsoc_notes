...
* The same chucking approach can also be applied to any DSP kernel
  that is connected to the data converter block's `axis` interface,
  using the ADC sample stream as a signal source. For example, one may
  modify the direct-form FIR filter implementation discussed in
  {numref}`sec:fir_direct` as below to filter the ADC samples directly
  from the data converter block:
